...
export default function KycReview() {
  ...
  const handleSubmit = async () => {
    ...
    if (!personal || !docs || !docs.selfie || !docs.idFront) {
      alert("Missing required data: make sure personal info and required documents are present.");
      return;
    }
    ...
    try {
      ...
      // Mark as submitted
      localStorage.setItem("kyc_submitted_v1", JSON.stringify({ submittedAt: new Date().toISOString() }));
      // Optionally you could clear inputs:
      // localStorage.removeItem(PERSONAL_KEY);
      // localStorage.removeItem(DOCS_KEY);

      // Friendly confirmation UI (non-blocking)
      alert("KYC submitted (demo). The real implementation should POST files to your server.");
    } catch (err) {
      console.error(err);
  ...
  if (!personal && !docs) {
    return (
      <Box>
        <Typography variant="body1">No KYC data found. Please complete previous steps.</Typography>
      </Box>
    );
  }
      ...
      <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
        ...
        <Grid container spacing={1}>
          <Grid item xs={12} sm={6}>
            <Typography variant="body2" sx={{ color: "text.secondary" }}>Full name</Typography>
          ...
          <Grid item xs={12}>
            <Typography variant="body2" sx={{ color: "text.secondary" }}>ID number</Typography>
        ...
        <Grid container spacing={2} alignItems="center">
          {docs?.selfie ? (
            <Grid item xs={12} sm={4}>
              ...
              <Box sx={{ mt: 1 }}>
                <Avatar src={docs.selfie.dataUrl} alt="selfie" sx={{ width: 120, height: 120 }} />
                <Typography variant="body2" sx={{ mt: 1 }}>{docs.selfie.name}</Typography>
              </Box>
            </Grid>
          ) : null}
          ...
          {docs?.idFront ? (
            <Grid item xs={12} sm={4}>
              ...
              <Box sx={{ mt: 1 }}>
                <Avatar src={docs.idFront.dataUrl} alt="id front" variant="rounded" sx={{ width: 120, height: 80 }} />
                <Typography variant="body2" sx={{ mt: 1 }}>{docs.idFront.name}</Typography>
              </Box>
            </Grid>
          ) : null}
          ...
          {docs?.idBack ? (
            <Grid item xs={12} sm={4}>
              ...
              <Box sx={{ mt: 1 }}>
                <Avatar src={docs.idBack.dataUrl} alt="id back" variant="rounded" sx={{ width: 120, height: 80 }} />
                <Typography variant="body2" sx={{ mt: 1 }}>{docs.idBack.name}</Typography>
              </Box>
            </Grid>
          ) : null}
          ...
          {docs?.other ? (
            <Grid item xs={12} sm={4}>
              ...
              <Box sx={{ mt: 1 }}>
                <Avatar src={docs.other.dataUrl} alt="other" variant="rounded" sx={{ width: 120, height: 80 }} />
                <Typography variant="body2" sx={{ mt: 1 }}>{docs.other.name}</Typography>
              </Box>
            </Grid>
          ) : null}
        </Grid>
